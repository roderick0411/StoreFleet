...
export const getAllProducts = async (req, res, next) => {
  ...
  try {
    ...
    const filterOptions = {};
    if (keyword) {
      const re = new RegExp(`[\w\d]*${keyword}[\w\d]*`);
      filterOptions.name = { $regex: re, $options: "i" };
    }
    if (minPrice) {
    ...
    const products = await getAllProductsRepo(filterOptions, page);
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    const reviewUserId = reviews[isReviewExistIndex].user;
    const userId = req.user._id.toString();
    console.log(reviewUserId);
    console.log(userId);
    if (reviewUserId != userId) {
      return next(
        new ErrorHandler(400, "You are not the author of this review")
      );
    }
